...
# 根据多个股票池进行一次完整的单因子测试
def sf_test_multiple_pools(factor=None, *, direction='+', bb_obj=None, discard_factor=[], holding_freq='w',
                           ...
                           do_data_description=False, do_factor_corr_test=False, loc=0):
...
def sf_test_multiple_pools_parallel(factor=None, *, direction='+', bb_obj=None, discard_factor=[],
                                    ...
                                    holding_freq='w', do_data_description=False, loc=0):
    cp_adj = data.read_data(['ClosePrice_adj'])
    cp_adj = cp_adj['ClosePrice_adj']
    temp_position = position(cp_adj)
...
sf_test_multiple_pools(factor='runner_value_36', direction='+', bkt_start=pd.Timestamp('2010-04-02'), holding_freq='w',
