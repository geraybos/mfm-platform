...
class xy_base_prod(xy_base):
    ...
    def save_factor_expo_to_sql(self):
        ...
        for item, df in style_factor_expo.iteritems():
            ...
            # 储存到数据库中
            self.rm_engine.insert_df(sql_df, 'BarraBaseFactorExpo')
            # 打印储存成功的提示
    ...
    def delete_from_sql_table(self, stock_pool, *, table_name='BarraBaseFactorExpo', factor_name=None,
                              data_date=None):
    ...
    def update_factor_base_data(self, *, start_date=None):
        ...
        last_day = self.rm_engine.get_original_data("select max(DataDate) from BarraBaseFactorExpo where "
        ...
        sql_dropped_time = "select distinct DataDate from BarraBaseFactorExpo where DataDate >= '" + \
    ...
    def save_cov_mat_to_sql(self):
        ...
        sql_df['StockPool'] = self.base_data.stock_pool
        # 储存到数据库中
        self.rm_engine.insert_df(sql_df, 'BarraBaseCovMat')
    ...
    def save_spec_var_to_sql(self):
        ...
        sql_df['StockPool'] = self.base_data.stock_pool
        # 储存到数据库中
        self.rm_engine.insert_df(sql_df, 'BarraBaseSpecVar')
    ...
    def save_daily_factor_var_to_sql(self):
        ...
        sql_df['StockPool'] = self.base_data.stock_pool
        # 储存到数据库中
        self.rm_engine.insert_df(sql_df, 'BarraBaseDailyFactorVar')
        print('Daily Factor Var has been successfully saved into sql database!\n')

    # 储存每日的每支股票特定风险
    def save_daily_spec_var_to_sql(self):
        self.daily_spec_var.index.rename('DataDate', inplace=True)
        sql_df = pd.melt(self.daily_spec_var.reset_index(level=0), id_vars='DataDate', var_name='SecuCode',
                         value_name='Value').dropna()
        sql_df['StockPool'] = self.base_data.stock_pool
        # 储存到数据库中
        self.rm_engine.insert_df(sql_df, 'BarraBaseDailySpecVar')
