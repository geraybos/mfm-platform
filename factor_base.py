...
class factor_base(object):
    ...
    def get_initial_spec_vol(self, *, sample_size=360, spec_var_half_life=84, nw_lag=5, forecast_steps=21):
        ...
        for cursor, time in enumerate(valid_data.index):
            # if cursor < sample_size - 1:
            if time < pd.Timestamp('2014-12-05'):
                continue
    ...
    def get_vra_spec_vol(self, *, sample_size=252, vra_half_life=42):
        ...
        standardized_spec_ret = self.specific_return.div(daily_spec_vol.shift(1))
        ...
        self.vra_spec_vol = spec_vol.mul(self.vra_multiplier_spec, axis=0)
    ...
    def get_bayesian_shrinkage_spec_vol(self, *, shrinkage_parameter=0.1):
        ...
        bs_spec_vol = initial_spec_vol * np.nan

        # 按照日期进行循环
        for time, curr_spec_vol in initial_spec_vol.iterrows():
            ...
            pass
        pass
        bs_spec_vol.to_hdf('bb_factor_bsspecvol_all', '123')
